...
The AASB generator evaluates **required information elements**, rather than
...
`ESG_DB_PATH` is a legacy environment-variable name retained for backward
compatibility. Despite the name, it points to the active SQLite database used by
the current AASB-only application.
...
The export path is optional and defaults to the AASB export filename shown. Exports overwrite
...
The filename `esg_reports.db` is a legacy name retained to avoid unnecessary
migration risk. It is the active database for the current AASB-only application.
...
by Git. Back up with SQLite's backup API.

For the hackathon deployment, ephemeral storage is acceptable because permanent
retention is not required. A production deployment should use persistent storage
and backups.
...
Other formats can be stored but extraction fails clearly. No paid service, vector
database, OCR, authentication, distributed queue, cancellation or automatic
recovery after a killed process is included. SQLite remains the source of truth.

## Gemini pacing and retry policy

Successful active process commands automatically export the AASB S2 report JSON to
...
The active AASB report generator selects numbered source excerpts instead of writing quotations.
...
full-report citation-correction call. The active AASB report uses one generation request.
...
requests and 1 AASB report request: approximately 14 calls instead of 98. This excludes
...
Deployment is still pending. For the hackathon, ephemeral storage is acceptable because
permanent retention is not required. Before production use, add authentication and
company-level authorization, durable job execution/recovery, persistent storage and
backups, production URL/CORS configuration, and an agreed Gemini quota/billing
arrangement.