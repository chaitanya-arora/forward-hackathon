...
export async function exportCompletedReports(result, root = reportsRoot) {
  ...
  try {
    ...
    throw new Error(`Run ${result.runId} completed and its AASB report is saved in SQLite, but file export failed: ${error.message}. Use export-aasb ${result.companyId} ${result.reportIds.aasbS2} to retry export without regenerating the report.`);
  }
